...
public class ReminderManager extends PreferencesFile{
	...
	public void checkAll(){
		for(int i = 0; i<reminders.size();i++){
			if (reminders.get(i).checkDue()){
				new ReminderButtonListener().operation();
			}
		}
	}
	/*
	...
	private void removeDuplicate(String url, String user){
		ArrayList<Integer> toRemove = new ArrayList<Integer>();
		for (Reminder rem : reminders){
			ArrayList<Reminder> tempReminders = reminders;
			tempReminders.remove(rem);
			if (reminders.indexOf(rem) == reminders.size()-1)
				break;
			for (int i = reminders.indexOf(rem); i < tempReminders.size(); i++){
				Reminder tempRem = tempReminders.get(i);
				if (rem.getUrl().equals(tempRem.getUrl()) && rem.getUser().equals(tempRem.getUser())){
					toRemove.add(reminders.indexOf(rem));
					break;
				}
			}
		}
